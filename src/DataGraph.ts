...
export class DataGraph {
  ...
  addNodes(nodes: Set<DataNode>) {
    this.forEachInTree(nodes, TreeTraversalOrder.PRE, (node) => {
      ...
      // map.
      if (node.actionHandlers) {
        for (const key in node.actionHandlers) {
  ...
  removeNodes(nodes: Set<DataNode>) {
    ...
    this.forEachInTree(nodes, TreeTraversalOrder.POST, (node) => {
      invariant(this.nodeContextMap.has(node), 'node must be in the graph');
  ...
  /**
   * Calls action handlers for all nodes registered with the action type,
   */
  handleAction(dispatch: Dispatch, action: AnyAction): Set<DataNode> {
    ...
    listenerSet.forEach((listener: DataNode) => {
      const actionHandlers = listener.actionHandlers as ActionHandlers;
      invariant(
        listener.actionHandlers,
        `Action ${action.type} registered as listener for ${listener.constructor} instance, but no \
        actionHandlers object exists`,
      );
  ...
  /**
   * Recursively collect all dependencies in the graph for nodes that have changed
   * after a call to `updateValue`.
   *
